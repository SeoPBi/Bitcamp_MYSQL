...
-- 지금까지 배운 문법으로 수도권(서울, 경기)에 사는 사람을 한 줄의 쿼리문으로 하나로 조회해주세요.
SELECT * FROM user_tbl WHERE user_address = '서울' OR user_address = '경기';

-- IN문법은 IN다음에 오는 리스트 목록 중 포함된 요소만 출력해줍니다.
SELECT * FROM user_tbl WHERE user_address IN ('서울', '경기');

-- 테이블 조회
SELECT * FROM buy_tbl;

-- IN을 응용해서 구매내역이 있는 유저만 출력해보겠습니다.
...
-- AND를 이용해서도 동일한 조건의 쿼리문을 작성해보세요.
...
SELECT * FROM user_tbl WHERE user_height IS NULL; -- 결과 나옴 
