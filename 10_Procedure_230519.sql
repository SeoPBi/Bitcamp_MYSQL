...
프리시저 : 매개변수는 존재하나 반환값이 없고, 특정 로직만 처리할때 주로 선언 
DELIMETER는 JAVA의 Main Method와 같은 역할을 함

DELIMITER $$ -- 시작지점
CREATE PROICEDURE 선언할프로시저명()
...
BEGIN
    ...
    SET var1 = 100; # 위에 var1 INT로 선언된 변수에 100 대입
...
-- 평균보다 키 큰 사람 전체목록을 호출해주는 프로시저입니다.
DELIMITER $$
...
-- 특정 프로시저는 DB에 소속된 것이기 때문에 반드시 DB가 먼저 선텍되어야 명령을 수행합니다.
...
BEGIN
    ...
    if(point) >= 90 THEN
		SELECT CONCAT('취득점수:',point,' 학점 : A');
    elseif(point) >= 80 THEN   
		SELECT CONCAT('취득점수:',point,' 학점 : B');
	elseif(point) >= 70 THEN   
		SELECT CONCAT('취득점수:',point,' 학점 : C');
	elseif(point) >= 60 THEN   
		SELECT CONCAT('취득점수:',point,' 학점 : D');
	else 
		SELECT CONCAT('취득점수:',point,' 학점 : F');
	END IF;
END $$
DELIMITER ;
...
/* 3 + 6 + 9 + 12 + ... 와 같은 방식으로
3부터 99까지 더했을때, 총합을 SELECT구문으로 프로시저 작성 후 호출
프로시저명은 sum3nProc() */
DELIMITER $$
...
/* 프로시저 getThree()의 문제점은 조회하려는 번호를 바꿀때마다 
