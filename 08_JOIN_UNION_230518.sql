/**
    ...
    SELECT 테이블1.컬럼1, 테이블2.컬럼2 ... 테이블2.컬럼1, 테이블2,컬럼2...
...
INSERT INTO purchase_tbl VALUES 
	...
	(null, 1, '2023-05-12', 40000),
	(null, 1, '2023-05-12', 30000),
	(null, 3, '2023-05-12', 30000),
...
-- buy_tbl과 user_tbl이 원래 있었는데, 그 둘을 조인해주세요.
-- 이 문제에 대해서도 테이블 별칭을 쓰는 구문으로 바꿔서 작성해보세요.alter
SELECT u.user_name, u.user_address, u.user_num,
	b.buy_num, b.prod_name, b.prod_cate, b.price,
	b.amount
...
-- 따라서 뒤에서 배울 UNION이란느 구문을 응용해 처리합니다.
...
SELECT u.user_num, b.buy_num FROM
	user_tbl u, buy_tbl b; -- ,로 나열 시 cross join 구문 생략 가능
-- user_tbl 10개 로우, buy_tbl은 3개 로우 = 크로스조인 결과 로우는 30개
SELECT COUNT(*) FROM
...
INSERT INTO phone_color VALUES
    ...
    ('노란색'),
    ('회색');
SELECT * FROM phone_color;

-- 여러분들이 직접 크로스조인 결과를 보면서 모든 조합쌍이 나왔는지 체크해주세요.
SELECT
	volume, model_name, color
FROM phone_volume
CROSS JOIN phone_color;

-- SELF조인은 자기 테이블 내부 자료를 참조하는 컬럼이 있을때
-- 해당 자료를 온전하게 노출시키기 위해서 사용하는 경우가 대부분입니다.
-- 예시로는 사원 목록 중 자기 자신과 직속 상사를 나타내거나 게시판에서 원본글과 답변글을 나타내는 경우
...
FROM
	staff as l LEFT JOIN staff as r
ON
